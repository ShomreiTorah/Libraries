...
namespace ShomreiTorah.Common.Calendar.Zmanim {
	...
	public class CalculatingZmanimProvider : YearlyZmanimProvider {
		...
		Dictionary<Zman, TimeSpan> CalculateZmanim(DateTime date) {
			...
			retVal.Add(Zman.Sunrise, CalcSunTime(date, 545 / 6, true));
			retVal.Add(Zman.Sunset, CalcSunTime(date, 545 / 6, false));
			...
			retVal.Add(Zman.סוף٠זמן٠קריאת٠שמע٠מ٠א, retVal[Zman.Sunrise] + TimeSpan.FromTicks(hour2 * 3));
		...
		TimeSpan CalcSunTime(DateTime date, double zenith, bool sunrise) {
			...
			var N3 = (1 + Math.Floor((date.Year - 4 * Math.Floor(date.Year / 4.0) + 2) / 3));
